-- Corrige referência ambígua a "card_name" na função auto_create_credit_card_with_holder
-- Data: 2025-08-18
...
BEGIN
  ...
  WHERE user_id = p_user_id 
    ...
    AND (
      ...
      OR (p_last_four_digits IS NULL AND credit_cards.card_name ILIKE '%' || p_card_holder || '%')
    )
  LIMIT 1;
...
COMMENT ON FUNCTION auto_create_credit_card_with_holder IS 'Cria cartões de crédito com informação do portador, suportando cartões dependentes. (ambiguidade corrigida)';
