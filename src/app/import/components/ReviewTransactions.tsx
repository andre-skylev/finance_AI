...
interface ReviewTransactionsProps {
  ...
  cardInfo?: CardInfo
  creditCardsInfo?: CreditCardInfo[] // Nova prop para cartões dependentes
  installmentDetails?: Record<string, any> // Nova prop para detalhes dos parcelamentos
  detectedBank: string
  documentType: string
  ...
  return (
    <div className="w-full max-w-4xl mx-auto space-y-6">
      <div className="p-6 bg-white rounded-lg shadow">
        ...
        {/* Informações dos cartões (incluindo dependentes) */}
        {isCreditCard && creditCardsInfo && creditCardsInfo.length > 1 && (
          <div className="mb-6 p-4 bg-purple-50 border border-purple-200 rounded-lg">
            <h4 className="font-semibold text-purple-800 mb-3 flex items-center">
              <CreditCard className="h-5 w-5 mr-2" />
              Cartões Identificados ({creditCardsInfo.length})
            </h4>
                ...
                <div key={index} className="flex justify-between items-center p-2 bg-white rounded border">
                  <div>
                    <p className="font-medium text-gray-800">
                      {card.cardHolder}
                      {card.isDependent && <span className="ml-2 text-xs bg-blue-100 text-blue-700 px-2 py-1 rounded">Dependente</span>}
                    </p>
                    <p className="text-sm text-gray-600">{card.cardNumber}</p>
                  </div>
                  {card.sharedLimit && index === 0 && (
                    <div className="text-right">
                      <p className="text-sm text-gray-600">Limite Compartilhado</p>
                      <p className="font-semibold text-gray-800">€{card.sharedLimit.toFixed(2)}</p>
                    </div>
                  )}
                </div>
              ))}
